...
Walks year-by-year from ACCOUNT_START_YEAR so streaks/totals aren't clipped
to a rolling 12-month window. No token, no GraphQL.
"""
import datetime
import json
import os
...
def fetch_window(from_date, to_date):
    ...
    if not cells:
        print(
            f"no calendar cells for {from_date}..{to_date} -- markup may have changed",
            file=sys.stderr,
        )
        return {}
...
def fetch_all_days():
    ...
    for year in range(ACCOUNT_START_YEAR, today.year + 1):
        ...
        got = {d: c for d, c in got.items() if d[:4] == str(year) and d <= today_s}
        merged.update(got)
        print(
            f"  {year}: {len(got)} days, {sum(got.values())} contributions",
            file=sys.stderr,
        )
...
def build_data(all_days):
    ...
    grid_days = [
        d for d in all_days if datetime.date.fromisoformat(d["date"]) >= window_start
    ]
...
if __name__ == "__main__":
    ...
    print(
        f"wrote {OUT_PATH}: {data['total_contributions']:,} all-time contributions "
